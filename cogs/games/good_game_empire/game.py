...
async def add_member_to_alliance(interaction: discord.Interaction, member: discord.Member, ingame_name: str):
    ...
    if player_result:
        await interaction.response.send_message(f"{dc_name} ist bereits in der Allianz registriert.")
    else:
   ...
   # cursor.close()
    #conn.close()
    await interaction.response.send_message(f"{dc_name} wurde in der Allianz registriert.")

async def remove_member_from_alliance(interaction: discord.Interaction, member: discord.Member):
    guild_id = interaction.guild.id
    conn = mysql.connector.connect(
            host=db_host,
            port=db_port,
...
async def add_place_to_member(interaction: discord.Interaction, place_type: str, place_name: str, coords: str, world_name: str):
    # Hole die Allianz-ID basierend auf der GUILD ID
    guild_id = interaction.guild.id
...
async def remove_place_from_member(interaction: discord.Interaction,place_name: str, world_name: str):
    # Hole die Allianz-ID basierend auf der GUILD ID
        guild_id = interaction.guild.id
...
async  def report_attack(interaction: discord.Interaction, world: str, defender_name: str,
